const { buildEventQuery, buildSessionQuery, buildSessionDurationQuery } = require('./queries')
...
function buildQuery ({ metric, dimension }, options) {
  ...
  return buildSessionDurationQuery({ metric, dimension, ...options })
}

function translateToGeoJson (redshiftData) {
  console.log(typeof redshiftData[0].timestamp)
