  ...
  return {
    ...
    coerce (value, state, options) {
      ...
      if (!isValidTimeRangeArray(timeRange)) {
        return this.createError('"time" param must be a comma delimited string: "<start>,<end>". Use "null", an ISO Date srting, YYYY-MM-DD string, or a unix timestamp', { v: value }, state, options)
      }

      return translateTimeRangeArray(timeRange)
    }
  }
}
